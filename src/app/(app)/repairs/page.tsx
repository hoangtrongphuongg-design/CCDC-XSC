        ...
        <Card className="table-card">
          ...
          <CardContent>
            <DataTable headers={["Phiếu", "Máy", "Nhóm báo", "Nội dung hư", "Chi phí", "Trạng thái", "Thao tác"]} rows={rows.map((row) => {
              ...
              if (row.status === "repairing" && auth.isWsManager) actions.push(<form action={completeRepairAction} key="done" className="row-actions"><input type="hidden" name="repairId" value={row.id} /><select name="result" aria-label="Kết quả" className="field-inline-md"><option value="completed">Sửa đạt</option><option value="irreparable">Không phục hồi</option></select><input name="cost" type="number" min="0" step="1000" defaultValue="0" aria-label="Chi phí" className="field-inline-sm" /><input name="workDescription" placeholder="Nội dung sửa" aria-label="Nội dung sửa" className="field-inline-md" /><input name="resultNotes" placeholder="Kết quả thử" aria-label="Kết quả thử" className="field-inline-md" /><Button size="sm">Hoàn tất sửa</Button></form>);

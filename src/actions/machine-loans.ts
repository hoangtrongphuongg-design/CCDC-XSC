...
export async function createMachineLoanAction(formData: FormData) {
  ...
  if (!equipmentId || !borrowerGroupId || !purpose || !expectedReturnDate) throw new Error("Vui lòng nhập đủ thông tin mượn máy.");
  const auth = await requireGroupPermission(borrowerGroupId, "operator");
  const [borrowerGroup] = await db.select().from(groups).where(eq(groups.id, borrowerGroupId)).limit(1);
  if (!borrowerGroup || borrowerGroup.isSystem || !borrowerGroup.isActive) throw new Error("Nhóm mượn không hợp lệ.");

  await db.transaction(async (tx) => {
    const item = await lockEquipment(tx, equipmentId);
    if (item.owner_group_id === borrowerGroupId) throw new Error("Không cần lập phiếu mượn cho máy thuộc chính nhóm.");
    if (item.status !== "in_use_owner") throw new Error("Máy hiện không sẵn sàng để cho mượn.");
    ...
    await writeAudit(tx as never, {
      ...
      entityId: created.id,
      description: `Tạo đề xuất mượn máy ${item.code}`,
      afterData: created,
    });
  });
  refresh();
...
export async function approveMachineLoanAction(formData: FormData) {
  ...
  const auth = await requireGroupPermission(loan.ownerGroupId, "manager");
  if (loan.requestedBy === auth.userId) throw new Error("Người tạo phiếu không được tự duyệt phiếu.");
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.update(equipment).set({ status: "wait_handover", updatedAt: new Date() }).where(eq(equipment.id, loan.equipmentId));
    await writeAudit(tx as never, {
      actorUserId: auth.userId,
      ...
      entityType: "machine_loan",
      entityId: loan.id,
      description: `Duyệt phiếu ${loan.code}`,
      beforeData: loan,
      afterData: updated,
    });
  });
...
export async function confirmLoanHandoverAction(formData: FormData) {
  ...
  await db.transaction(async (tx) => {
    const [updated] = await tx.update(machineLoans).set({ handedOverBy: auth.userId, handedOverAt: new Date(), updatedAt: new Date() })
...
export async function confirmLoanReceiptAction(formData: FormData) {
  ...
  if (!loan.handedOverAt || loan.status !== "wait_handover") throw new Error("Bên cho chưa xác nhận bàn giao.");

  await db.transaction(async (tx) => {
    await lockEquipment(tx, loan.equipmentId);
    const [updated] = await tx.update(machineLoans).set({ status: "on_loan", receivedBy: auth.userId, receivedAt: new Date(), updatedAt: new Date() })
      .where(and(eq(machineLoans.id, loan.id), eq(machineLoans.status, "wait_handover"))).returning();
    if (!updated) throw new Error("Phiếu đã được xử lý.");
    await tx.update(equipment).set({ status: "on_loan", currentGroupId: loan.borrowerGroupId, currentHolderId: auth.userId, updatedAt: new Date() })
    ...
    await writeAudit(tx as never, {
      ...
      entityType: "machine_loan",
      entityId: loan.id,
      description: `Xác nhận nhận máy theo phiếu ${loan.code}`,
      afterData: updated,
    });
  });
  refresh();
...
export async function requestMachineReturnAction(formData: FormData) {
  ...
  await db.transaction(async (tx) => {
    await lockEquipment(tx, loan.equipmentId);
    await assertEquipmentHasNoOtherOpenWorkflow(tx, loan.equipmentId, { machineLoanId: loan.id });
    const [updated] = await tx.update(machineLoans).set({ status: "return_requested", returnRequestedBy: auth.userId, returnRequestedAt: new Date(), updatedAt: new Date() })
    ...
    await tx.update(equipment).set({ status: "return_requested", updatedAt: new Date() }).where(eq(equipment.id, loan.equipmentId));
    await writeAudit(tx as never, {
      actorUserId: auth.userId,
      actorGroupId: loan.borrowerGroupId,
      ...
      entityType: "machine_loan",
      entityId: loan.id,
      description: `Báo trả máy theo phiếu ${loan.code}`,
      afterData: updated,
    });
  });
  refresh();
...
export async function confirmMachineReturnAction(formData: FormData) {
  ...
  await db.transaction(async (tx) => {
    await lockEquipment(tx, loan.equipmentId);
    await assertEquipmentHasNoOtherOpenWorkflow(tx, loan.equipmentId, { machineLoanId: loan.id });
    const [updated] = await tx.update(machineLoans).set({ status: "completed", closedBy: auth.userId, closedAt: new Date(), returnCondition: returnCondition || null, updatedAt: new Date() })
    ...
    await writeAudit(tx as never, {
      ...
      entityType: "machine_loan",
      entityId: loan.id,
      description: `Xác nhận nhận lại máy theo phiếu ${loan.code}`,
      afterData: updated,
    });
  });
  refresh();
